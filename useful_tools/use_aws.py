...
2. Flexible Authentication:
   • Direct credential passing (access key, secret key, session token)
   • AWS profile support for credential management
   • Shared memory integration for retrieving credentials
   • Environment variable fallback for standard AWS credential chains
...
5. Usage Examples:
   ...
   # List S3 buckets with default credentials
   ...
   # Using shared memory credentials
   result = agent.tool.use_aws(
       service_name="s3",
       operation_name="list_buckets",
       parameters={},
...
def get_boto3_client(
    ...
    Args:
        ...
        aws_access_key_id: Optional AWS access key ID for direct credentials
        aws_secret_access_key: Optional AWS secret access key for direct credentials
        aws_session_token: Optional AWS session token for temporary credentials

    Returns:
        A boto3 client object for the specified service
    """
    # If direct credentials are provided, use them instead of profile
        ...
        "json": {
            ...
            "properties": {
                ...
                "aws_access_key_id": {
                    "type": "string",
                    "description": (
                        "Optional: AWS access key ID for direct authentication. "
                        "If provided with aws_secret_access_key, these will be used instead of profile_name."
                    ),
                },
                "aws_secret_access_key": {
                    "type": "string",
                    "description": (
                        "Optional: AWS secret access key for direct authentication. "
                        "Must be provided if aws_access_key_id is specified."
                    ),
                },
                "aws_session_token": {
                    "type": "string",
                    "description": (
                        ...
                        "Only required when using temporary credentials."
                    ),
                },
                "session_id": {
                    "type": "string",
                    "description": (
                        "Optional: Session ID to retrieve AWS credentials from shared memory. "
                        "If provided, the system will attempt to load credentials from shared memory."
                    ),
                },
                "session_key": {
                    "type": "string",
                    "description": (
                        "Optional: Session key for retrieving specific credentials from shared memory. "
                        "Used in conjunction with session_id."
                    ),
                },
            },
            "required": [
...
def use_aws(tool: ToolUse, **kwargs: Any) -> ToolResult:
    ...
    ---------------------
    1. Direct Credentials: Provide aws_access_key_id and aws_secret_access_key directly
    2. Profile: Use profile_name to reference credentials in ~/.aws/credentials
    3. Shared Memory: Provide session_id to retrieve credentials from shared memory
    4. Environment Variables: If no credentials are provided, boto3 will use AWS_* environment variables
    ...
    Args:
        tool: The ToolUse object containing:
            ...
            - input: Dictionary containing:
                ...
                - aws_access_key_id: Optional AWS access key ID for direct authentication
                - aws_secret_access_key: Optional AWS secret access key for direct authentication
                - aws_session_token: Optional AWS session token for temporary credentials
                - session_id: Optional session ID to retrieve credentials from shared memory
                - session_key: Optional session key for retrieving specific credentials
    ...
    # Get credentials from shared memory if session_id is provided
    aws_access_key_id = tool_input.get("aws_access_key_id")
    aws_secret_access_key = tool_input.get("aws_secret_access_key")
    aws_session_token = tool_input.get("aws_session_token")
    session_id = tool_input.get("session_id")
    
    # Try to get credentials from shared memory if session_id is provided
    if session_id and not (aws_access_key_id and aws_secret_access_key):
        try:
            # Import shared_memory here to avoid circular imports
            from shared_memory import shared_memory
            
            # Get AWS credentials from shared memory
            aws_access_key_id = shared_memory.get("aws_access_key_id", session_id=session_id)
            aws_secret_access_key = shared_memory.get("aws_secret_access_key", session_id=session_id)
            aws_session_token = shared_memory.get("aws_session_token", session_id=session_id)
            
            if aws_access_key_id and aws_secret_access_key:
                logger.info(f"Retrieved AWS credentials from shared memory for session {session_id}")
            else:
                logger.warning(f"No AWS credentials found in shared memory for session {session_id}")
        except ImportError:
            logger.warning("Could not import shared_memory module")
        except Exception as e:
            logger.error(f"Error retrieving AWS credentials from shared memory: {e}")
    
    # Set up the boto3 client with credentials if available
